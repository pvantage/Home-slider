...
if(isset($_POST['registration']))
{
	...
	if(count($error)<=0)
	{
		$sql="INSERT INTO `".$prefix."homeslides` (`title`, `detail`, `add_date`, `active`) VALUES ('$title', '$detail', now(), '1')";
		$result = $wpdb->query( $sql );
		if($result==1)
		{
			$lastid=$wpdb->insert_id;
			$aliasbeforeimagetitle='banner-'.$lastid;
			$file='beforeimage';
					...
					|| (strtolower($_FILES[$file]["type"]) == "image/pjpeg"))
					  {
						...
						else
						{
							...
							$sql="UPDATE `".$prefix."homeslides` set beforeimage='$altername' where id='$lastid'";
							$result = $wpdb->query( $sql );
						}
					}
				}
...
<script type="text/javascript" src="<?php echo get_option('home');?>/wp-content/plugins/home-slider/js/validate.js"></script>
        ...
        <div class="right donotshowerror">
        	<form action="" method="post" name="register_spcialist" id="register_spcialist" enctype="multipart/form-data">
                ...
                <div class="e-mail">
                    ...
                    <div class="field"><input type="file" name="beforeimage" class="required" /></div>
                </div>
                <div class="e-mail">
